...
public class CustomViewAbove extends ViewGroup implements SlidingEnablingInterface { 
	
	private static final String TAG = "CustomViewAbove"; 
	
	
	//////////////////////////////////////////////////////
	// ENABLED SLIDING MANAGEMENT 
	////////////////////////////////////////////////////// 
	
	/** 
	 * Calculate zone where sliding is operative. 
	 */ 
	private int heightEnabledZone; 
	
	/** 
	 * Open state 
	 */ 
	private int slidingEnabled = OPEN_RIGHT & OPEN_LEFT; 
	
	public void setHeightEnabledZone(int pixelHeight) { 
		heightEnabledZone = pixelHeight; 
	}
	...
	public void setSlidingEnabled(boolean enabled) { 
		if ( enabled ) { 
			slidingEnabled = OPEN_LEFT & OPEN_RIGHT; 
		} else { 
			slidingEnabled = NO_OPEN_LEFT & NO_OPEN_RIGHT; 
		}
	} 
	
	public boolean isSlidingEnabled() { 
		return (slidingEnabled & OPEN_LEFT & OPEN_RIGHT) != 0; 
	}
	
	public void setLeftSlidingEnabled(boolean enabled) { 
		if ( enabled ) { 
			slidingEnabled = OPEN_LEFT; 
		} else { 
			slidingEnabled = NO_OPEN_LEFT; 
		}
	} 
	
	public boolean isLeftSlidingEnabled() { 
		return (slidingEnabled & NO_OPEN_RIGHT) != 0; 
	}
	
	public void setRightSlidingEnabled(boolean enabled) { 
		if ( enabled ) { 
			slidingEnabled = OPEN_RIGHT; 
		} else { 
			slidingEnabled = NO_OPEN_RIGHT; 
		}
	} 
	
	public boolean isRightSlidingEnabled() { 
		return (slidingEnabled & NO_OPEN_LEFT) != 0; 
	}
	...
	/**
	 * @param context the context where this view is 
	 */
	...
	/** 
	 * @param context the context where this view is 
	 * @param attrs - set of attribute values. 
	 */
	...
	void initCustomViewAbove() { 
		...
		//Context and instance of access to resources  
	...
	/**
	 * 
	 * @param dx - absolute displacement of touch event 
	 * @return if slide is allowed or not. 
	 */
	private boolean thisSlideAllowed(float posY, float dx) { 
		...
		if ( !isMenuOpen() ) { 
			...
			// First of all if menu is closed determine if height is inside of enabled zone. 
			if ( posY >= heightEnabledZone ) { 
				return false; 
			} 
			
			// Second, if menu is closed determine the open side and if it is enabled. 
			if ( dx < 0 && !isRightSlidingEnabled() ) { 
				
				/////////////////////////
				// LOCK RIGHT MENU 
				/////////////////////////
				if (DEBUG)
					Log.v(TAG, "Right opening is disabled " + dx ); 
				return false;  
			} else if ( dx > 0 && !isLeftSlidingEnabled() ) { 
				
				/////////////////////////
				// LOCK LEFT MENU 
				/////////////////////////
				if (DEBUG)
					Log.v(TAG, "Left opening is disabled " + dx ); 
				return false; 
			} 
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) { 
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (xDiff > mTouchSlop && xDiff > yDiff && thisSlideAllowed( y, dx ) ) {
		...
		case MotionEvent.ACTION_MOVE:
			if (!mIsBeingDragged) {				
				...
				if ((xDiff > mTouchSlop || (mQuickReturn && xDiff > mTouchSlop / 4))
						&& xDiff > yDiff && thisSlideAllowed( y, dx )) {
